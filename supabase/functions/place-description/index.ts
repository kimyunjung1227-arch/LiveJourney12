...
// 지역 환각 방지: 전달된 지역을 '확정값'으로 못 박고, 모델이 아는 다른 지역명 사용을 금지.
// 같은 이름의 다른 지역 장소(예: 영천 vs 구미)와 혼동하는 문제를 막는다.
function buildRegionRule(regionHint: string): string {
  const r = String(regionHint || '').trim();
  if (r) {
    return (
      `- 이 장소의 위치(행정구역)는 반드시 "${r}"로만 쓰세요. 당신이 알고 있는 다른 시/군/구 이름은 절대 쓰지 마세요.\n` +
      `  같은 이름을 가진 다른 지역의 장소와 혼동하지 말고, 위치를 언급할 때는 "${r}" 외 다른 지역명을 쓰면 안 됩니다.`
    );
  }
  return (
    `- 지역 정보가 없습니다. 위치나 행정구역(시/군/구)을 추측해서 쓰지 마세요. 특정 지역명을 단정하지 말고 장소 자체의 특징만 설명하세요.`
  );
}

// 소개글 ↔ 장소 불일치 방지: 실제 게시물 글(캡션)을 근거로 삼고, 확신 없으면 단정 금지.
const GROUNDING_RULE =
  `- 아래 '분위기 힌트'는 그 장소에 실제로 올라온 게시물 글입니다. 이 장소가 어떤 곳인지(공원/호수/카페/거리/전망 등)와 지금 분위기를 이 글들과 일치하게 파악하고, 이와 모순되는 설명은 쓰지 마세요. 단, 글을 그대로 인용하거나 "제보/후기/리뷰"라고 언급하지는 마세요.\n` +
  `- 장소명과 지역만으로 그곳이 정확히 어디인지 확신이 서지 않으면, 유명세·랜드마크·구체적 사실을 단정하지 말고 위 게시물 글과 일반적인 특징 위주로 안전하게 쓰세요. (동명의 다른 유명 장소를 끌어와 설명하면 안 됩니다.)`;
...
function buildPrompt(input: {
  ...
  return (
    ...
    `${buildRegionRule(input.regionHint)}\n` +
    `${GROUNDING_RULE}\n` +
    `${buildSeasonLine()}\n` +
    `- 광고 문구/과장/이모지 금지. 존댓말.\n` +
