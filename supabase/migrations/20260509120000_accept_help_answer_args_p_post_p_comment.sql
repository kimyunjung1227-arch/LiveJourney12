-- LiveJourney: accept_help_answer RPC 인자명을 p_post / p_comment 로 통일
-- - PostgREST 요청 본문에서 `comment` 키가 일부 환경에서 문제를 일으키는 경우를 피함
-- - 본문 로직은 20260430225000_accept_help_answer_returns_json.sql 과 동일 (예외 시 JSON 반환, HTTP 400 방지)
-- - Postgres는 CREATE OR REPLACE 로 매개변수 이름(post→p_post)만 바꿀 수 없어 DROP 후 재생성

drop function if exists public.accept_help_answer(uuid, uuid);

create function public.accept_help_answer(p_post uuid, p_comment uuid)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  where c.id = p_comment and c.post_id = p_post;
  ...
  values (p_post, p_comment, uid, answer_user);
