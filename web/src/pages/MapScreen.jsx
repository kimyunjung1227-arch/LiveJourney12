...
function MapControls({ onMyLocation, onOpenList, bottom }) {
  return (
    <div
    ...
    >
      ...
      >
        <IconList size={20} color={KEY} stroke={2} />
      </button>
      <button
        type="button"
...
// 핀 좌표의 실시간 기온 (기상청 초단기실황 · 격자 5분 캐시)
function usePinWeather(lat, lng) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await getWeatherByCoords(lat, lng);
...
const MapScreen = () => {
  ...
  // 목록보기 — 현재 보고 있는 지도의 핀들을 별도 화면(지역 화면 톤)으로 모아 본다
  const handleOpenList = useCallback(() => {
    ...
    const goto = (regionLabel) =>
      navigate('/map/list', { state: { ...payload, regionLabel } });

    // 지도 중심의 행정구역명을 화면 제목으로 (실패/지연되면 제목 없이 진행)
    if (!center || !window.kakao.maps.services?.Geocoder) {
      goto('');
      return;
    }
    try {
      ...
      geocoder.coord2RegionCode(center.lng, center.lat, (result, status) => {
        ...
        if (
          ...
          if (r) {
            label = [r.region_1depth_name, r.region_2depth_name]
              .filter(Boolean)
              .join(' ');
          }
        }
  ...
  return (
    <div className="relative w-full h-[100dvh] overflow-hidden font-sans bg-[#EDF3F7]">
      ...
      <MapControls
        ...
        bottom={
          (searchedPlace || searchedRegion) && !selectedBundleId ? 196 : 24
        }
      />
